...
const getKVData = async (): Promise<DataItem[]> => {
  ...
  const promises = keys.map(key =>
    MY_KV_STORE.get(key, "text").then(data => JSON.parse(data || '[]'))   // JSON.parse()が文字列内の数値も適切に数値型に変換する能力を持っているため "100" は数値の 100 にパースされる
  );
  ...
  for (let i = 0; i < length; i++) {
    ...
    keys.forEach((key, index) => {
      item[key] = values[index][i] !== undefined ? values[index][i] : null;
    });
    data.push(item);
  }
